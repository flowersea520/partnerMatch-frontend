...
import {listMyJoinTeams} from "../../api/api.ts"
...
const onSearch = async (val) => {
  ...
  const result = await listMyJoinTeams(val);
...
const onClickButton = async () => {
  // 调用后端搜索队伍列表的接口，它有个属性searchText，
  // 如果传入了这个属性字段，那么它就会根据这个属性字段的值，去查数据库，根据数据库的 队伍名和队伍描述进行 like 模糊查询
  const result = await listMyJoinTeams(searchText.value);
  if (result?.code === 0 && result.data) {
    // 请求队伍列表成功, 将相应的结果，给我们的数据模型
    teamList.value = result.data;
    showSuccessToast("搜索队伍成功");
...
onMounted(async () => {
  // 调用封装好的查询队伍列表
  const result = await listMyJoinTeams();
  if (result?.code === 0 && result.data) {
    // 请求队伍列表成功, 将相应的结果，给我们的数据模型
    teamList.value = result.data;
